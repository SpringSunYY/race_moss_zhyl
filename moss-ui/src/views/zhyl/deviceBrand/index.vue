<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="deviceBrandList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="品牌名称" :show-overflow-tooltip="true" align="center" v-if="columns[1].visible"
                       prop="brandName"/>
      <el-table-column label="品牌描述" :show-overflow-tooltip="true" align="center" v-if="columns[2].visible"
                       prop="description"/>
      <el-table-column label="联系方式" :show-overflow-tooltip="true" align="center" v-if="columns[3].visible"
                       prop="contactInfo"/>
      <el-table-column label="内容" :show-overflow-tooltip="true" align="center" v-if="columns[4].visible"
                       prop="contractContent"/>
      <el-table-column label="签订时间" align="center" v-if="columns[5].visible" prop="contractSignTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.contractSignTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
        </template>
      </el-table-column>
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {key: 2, label: '品牌描述', visible: false},
