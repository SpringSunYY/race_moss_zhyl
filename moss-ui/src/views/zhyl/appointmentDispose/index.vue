  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['zhyl:appointmentDispose:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['zhyl:appointmentDispose:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['zhyl:appointmentDispose:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['zhyl:appointmentDispose:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList" :columns="columns"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="appointmentDisposeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="编号" align="center" v-if="columns[0].visible" prop="disposeId" />
        ...
        <el-table-column label="预约" :show-overflow-tooltip="true" align="center" v-if="columns[2].visible" prop="appointmentId" />
        ...
        <el-table-column label="处理地址" :show-overflow-tooltip="true" align="center" v-if="columns[4].visible" prop="disposeAddress" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="预约" prop="appointmentId">
          <el-input v-model="form.appointmentId" placeholder="请输入预约" />
        </el-form-item>
        ...
        <el-form-item label="处理状态" prop="disposeStatus">
          <el-select v-model="form.disposeStatus" placeholder="请选择处理状态">
            <el-option
              v-for="dict in dict.type.yl_dispose_status"
              :key="dict.value"
              :label="dict.label"
...
<script>
import { listAppointmentDispose, getAppointmentDispose, delAppointmentDispose, addAppointmentDispose, updateAppointmentDispose } from "@/api/zhyl/appointmentDispose";

export default {
  name: "AppointmentDispose",
    ...
    return {
      ...
      columns: [
        { key: 0, label: '编号', visible: true },
          ...
          { key: 2, label: '预约', visible: true },
          { key: 3, label: '处理凭证', visible: true },
          { key: 4, label: '处理地址', visible: true },
          { key: 5, label: '处理人', visible: true },
      ...
      rules: {
        ...
        appointmentId: [
          { required: true, message: "预约不能为空", trigger: "blur" }
        ],
        disposeVoucher: [
  ...
  methods: {
    ...
    getList() {
      ...
      listAppointmentDispose(this.queryParams).then(response => {
        this.appointmentDisposeList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getAppointmentDispose(disposeId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.disposeId != null) {
            updateAppointmentDispose(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addAppointmentDispose(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除预约处理编号为"' + disposeIds + '"的数据项？').then(function() {
        return delAppointmentDispose(disposeIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('zhyl/appointmentDispose/export', {
        ...this.queryParams
      }, `appointmentDispose_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
